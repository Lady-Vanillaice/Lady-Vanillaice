...
function KassenbuchPage() {
  ...
  function rows() {
    return filtered.map((e) => [
      ...
      e.kunde,
      e.studio,
      dateLabel(e.anzahlung_datum),
      eur(e.anzahlung),
      e.anzahlung_method ?? "—",
      dateLabel(e.bar_datum),
      eur(e.bar),
      eur(e.gesamt),
  ...
  function exportPdf() {
    ...
    doc.setFontSize(17);
    doc.text("LADY VANILLA ICE — Kassenbuch", 45, 72);
    doc.setFontSize(10);
    doc.text(format(parseISO(`${month}-01`), "LLLL yyyy", { locale: de }), 45, 90);
    doc.text(`Erstellt: ${format(new Date(), "dd.MM.yyyy HH:mm", { locale: de })}`, 795, 72, { align: "right" });
    autoTable(doc, {
      startY: 122,
      margin: { left: 45, right: 35, top: 70, bottom: 35 },
      head: [["Termin", "Kunde", "Studio", "Anzahlung am", "Anzahlung", "Art", "Bar am", "Bar", "Gesamt", "Status", "Dauer"]],
      body: rows(),
  ...
  function exportCsv() {
    const head = ["Termin", "Kunde", "Studio", "Anzahlung erhalten am", "Anzahlung EUR", "Zahlungsart", "Bar erhalten am", "Bar EUR", "Gesamt EUR", "Status", "Dauer"];
    const raw = filtered.map((e) => [e.termin_datum, e.kunde, e.studio, e.anzahlung_datum ?? "", e.anzahlung.toFixed(2).replace(".", ","), e.anzahlung_method ?? "", e.bar_datum ?? "", e.bar.toFixed(2).replace(".", ","), e.gesamt.toFixed(2).replace(".", ","), statusLabel[e.status], e.notiz ?? ""]);
  ...
  function exportExcel() {
    const table = [
      ["Termin", "Kunde", "Studio", "Anzahlung erhalten am", "Anzahlung", "Zahlungsart", "Bar erhalten am", "Bar", "Gesamt", "Status", "Dauer"],
      ...rows(),
  ...
  return <>
    ...
    <section className="py-12"><div className="container-luxe max-w-[1500px] space-y-8">
      ...
      <div className="bg-card border border-champagne/20 overflow-hidden">
        {isLoading ? <div className="p-8">Lade…</div> : error ? <div className="p-8 text-bordeaux">Fehler beim Laden</div> : <div className="overflow-x-auto"><table className="w-full text-sm min-w-[1320px]">
          <thead><tr className="border-b border-champagne/20 text-[10px] uppercase tracking-widest text-vanilla/50">{["Termin", "Kunde", "Studio", "Anzahlung am", "Anzahlung", "Zahlungsart", "Bar erhalten am", "Bar", "Gesamt", "Status", "Dauer", ""].map(h => <th key={h} className="p-3 text-left">{h}</th>)}</tr></thead>
          <tbody>{filtered.map(e => <tr key={e.id} className="border-b border-champagne/10 align-top">
            ...
            <td className="p-3 whitespace-nowrap">{dateLabel(e.bar_datum)}</td><td className="p-3 tabular-nums">{eur(e.bar)}</td><td className="p-3 tabular-nums text-champagne">{eur(e.gesamt)}</td>
            <td className="p-3"><span className={`status-${e.status}`}>{statusLabel[e.status]}</span></td><td className="p-3 max-w-[180px] text-vanilla/65">{e.notiz ?? "—"}</td>
...
function AccountingDialog({ entry, onClose, onSave }: { entry: CashBookEntry; onClose: () => void; onSave: (data: any) => Promise<void> }) {
  ...
  async function save() {
    ...
    try {
      await onSave({
        id: entry.booking_id,
        anzahlung: Number(a.replace(",", ".")) || 0,
        anzahlung_method: method || null,
        anzahlung_paid_at: aDate || null,
        bar: Number(bar.replace(",", ".")) || 0,
        completed_at: full ? (entry.durchgefuehrt_datum || today()) : null,
        cash_received_at: barDate || null,
        fully_paid: full,
        status,
        note: null,
      });
  ...
  return <div className="fixed inset-0 z-[100] bg-black/80 grid place-items-center p-4"><div className="bg-card border border-champagne/40 max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
    ...
    <div className="grid md:grid-cols-2 gap-4">
      ...
      <Field label="Anzahlung erhalten am"><input type="date" value={aDate} onChange={e => setADate(e.target.value)} className="luxe-input" /></Field><Field label="Bar erhalten (€)"><input value={bar} onChange={e => setBar(e.target.value)} className="luxe-input" /></Field>
      <Field label="Bar erhalten am"><input type="date" value={barDate} onChange={e => setBarDate(e.target.value)} className="luxe-input" /></Field><Field label="Status"><select value={status} onChange={e => setStatus(e.target.value as CashBookEntry["status"])} className="luxe-input"><option value="open">Offen</option><option value="completed">Erledigt</option><option value="cancelled">Storniert</option></select></Field>
      <label className="md:col-span-2 flex gap-3 items-center border border-champagne/20 p-3"><input type="checkbox" checked={full} onChange={e => { setFull(e.target.checked); if (e.target.checked) { setStatus("completed"); setBarDate(v => v || (Number(bar) > 0 ? today() : "")); } }} /><span>Vollständig bezahlt</span></label>
...
function Stat({ label, value, gold = false }: { label: string; value: string; gold?: boolean }) {
  return <div className="bg-card border border-champagne/20 p-4"><div className="text-[10px] uppercase tracking-widest text-vanilla/45">{label}</div><div className={`text-xl mt-1 ${gold ? "text-champagne" : "text-vanilla"}`}>{value}</div></div>;
}
