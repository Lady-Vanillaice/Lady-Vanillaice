...
function KassenbuchPage() {
  ...
  const filtered = useMemo(() => data.filter((e) => {
    ...
    return matchesMonth && (!studioFilter || e.studio === studioFilter) && (!methodFilter || paymentMethod === methodFilter) && (!statusFilter || e.status === statusFilter) && (!search || haystack.includes(search.toLowerCase()));
  }).sort((a, b) => {
  ...
  const paymentLabel = (e: CashBookEntry) => e.source === "booking" ? "Buchung" : "Manuelle Zahlung";
  const paymentMethodText = (e: CashBookEntry) => e.source === "booking" ? [e.anzahlung > 0 ? depositText(e) : null, e.bar > 0 ? "Bar" : null].filter(Boolean).join(" · ") || "—" : depositText(e);
