...
function Buchung() {
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const playWorld = String(form.get("play_world") ?? "").trim();
    ...
    setErrorMsg(null);
    try {
      await submit({ data: { guest_name: name, guest_email: email, guest_phone: phone || null, message, requested_start: requestedStartIso, duration_minutes: durationMinutes, duration_label: durationLabel, age_confirmed: true, marketing_consent: marketingConsent } });
      if (phone) try {
        await notifyWhatsApp({ data: { guest_name: name, guest_email: email, guest_phone: phone, requested_start: requestedStartIso, requested_end: requestedEndIso, message } });
      } catch (whatsAppError) {
      ...
      setStatus("error");
      setErrorMsg(err instanceof Error ? err.message : tr("Anfrage konnte nicht gesendet werden. Bitte versuche es später erneut.", "Your request could not be sent. Please try again later."));
    }
  }

  return (
    <>
      <PageHeader eyebrow={tr("Buchung", "Booking")} title={<>{tr("Termin ", "Request ")}<em className="font-script gold-text not-italic">{tr("anfragen", "an appointment")}</em></>} intro={tr("Bitte fülle das Formular aus oder schreibe mir direkt. Alle Anfragen werden vertraulich behandelt.", "Please fill in the form or write to me directly. All inquiries are handled confidentially.")} />
        ...
        <div className="container-luxe grid lg:grid-cols-12 gap-12">
          <div className="lg:col-span-12">
            <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10 border-y border-champagne/15 py-5 text-sm text-vanilla/80">
              <div className="flex items-center gap-2.5"><Lock className="text-champagne" size={18} /><span>{tr("Diskret", "Discreet")}</span></div>
              <div className="flex items-center gap-2.5"><Clock className="text-champagne" size={18} /><span>{tr("Antwort binnen 24 h", "Reply within 24 h")}</span></div>
              <div className="flex items-center gap-2.5"><ShieldCheck className="text-champagne" size={18} /><span>{tr("Keine Vorkasse für Erstkontakt", "No prepayment for first contact")}</span></div>
            </div>
          </div>
          <div className="lg:col-span-7">
            <form onSubmit={onSubmit} className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div><label className="eyebrow block mb-2">{tr("Name oder Pseudonym", "Name or pseudonym")}</label><input name="name" required className="input-luxe" placeholder={tr("Wie darf ich Dich nennen?", "What should I call you?")} /></div>
                <div><label className="eyebrow block mb-2">{tr("E-Mail-Adresse", "Email address")}</label><input name="email" type="email" required className="input-luxe" placeholder={tr("diskret@beispiel.de", "discreet@example.com")} /></div>
              </div>
              <div><label className="eyebrow block mb-2">{tr("WhatsApp (optional)", "WhatsApp (optional)")}</label><input name="phone" type="tel" className="input-luxe" placeholder="+49 …" /></div>
              <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="border border-champagne/20 bg-champagne/[0.03] p-5 space-y-5">
                ...
                <div>
                  <label className="eyebrow block mb-2">{tr("Spielwelt (optional)", "World of play (optional)")}</label>
                ...
                <div><label className="eyebrow block mb-2">{tr("Erfahrung", "Experience")}</label><select name="experience" required defaultValue="" className="input-luxe"><option value="" disabled>{tr("Bitte auswählen", "Please select")}</option><option value={tr("Erste Session", "First session")}>{tr("Erste Session", "First session")}</option><option value={tr("Etwas Erfahrung", "Some experience")}>{tr("Etwas Erfahrung", "Some experience")}</option><option value={tr("Erfahren", "Experienced")}>{tr("Erfahren", "Experienced")}</option></select></div>
                <div><label className="eyebrow block mb-2">{tr("Vorlieben & Wünsche", "Preferences & wishes")}</label><textarea name="preferences" rows={4} required minLength={3} maxLength={450} className="input-luxe resize-y" placeholder={tr("Was wünschst du dir? Welche Fantasien, Rollen oder Praktiken interessieren dich?", "What would you like? Which fantasies, roles or practices interest you?")} /></div>
                <div><label className="eyebrow block mb-2">{tr("Tabus & klare Grenzen", "Taboos & hard limits")}</label><textarea name="taboos" rows={3} required minLength={2} maxLength={450} className="input-luxe resize-y" placeholder={tr("Was darf auf keinen Fall passieren?", "What must not happen under any circumstances?")} /></div>
                <div><label className="eyebrow block mb-2">{tr("Gesundheitliche Hinweise (optional)", "Health information (optional)")}</label><textarea name="health" rows={2} maxLength={150} className="input-luxe resize-y" placeholder={tr("Zum Beispiel Verletzungen, Allergien oder Kreislaufprobleme", "For example injuries, allergies or circulation problems")} /></div>
              ...
              <div className="border border-champagne/40 bg-champagne/5 p-4 text-xs text-vanilla/75 leading-relaxed"><div className="eyebrow mb-1.5 text-champagne">{tr("Längere Sessions", "Longer sessions")}</div>{tr("Ab 4 Stunden kann man auch so im Studio mit mir etwas vereinbaren. Gib einfach deinen Wunschtermin und einen Ausweichtermin an — ich melde mich persönlich zur Abstimmung. Für längere Sessions ab 4 Stunden können wir optional auch gerne einen anderen Tag wählen.", "From 4 hours upwards we can also arrange something at the studio outside the calendar. Just enter your preferred and an alternative date — I'll be in touch personally to coordinate. For sessions of 4 hours or more we can also choose a different day if you prefer.")}</div>
              {status === "sent" ? <p className="text-sm text-champagne leading-relaxed">{tr("Deine Anfrage ist bei mir eingegangen. Du erhältst gleich eine Bestätigung per E-Mail mit allen Zahlungs- und Absagebedingungen. Sobald die Anzahlung bei mir eingegangen ist, melde ich mich persönlich bei Dir, um die Details zu besprechen.", "Your request has been received. You'll get an email confirmation shortly with all payment and cancellation terms. As soon as the deposit reaches me, I'll be in touch personally to discuss the details.")}</p> : null}
              {status === "error" && errorMsg ? <p className="text-sm text-red-400 leading-relaxed">{errorMsg}</p> : null}
              <p className="text-xs text-vanilla/40 leading-relaxed">{tr("Deine Anfrage wird vertraulich behandelt. Eine Bestätigung erhältst Du per E-Mail.", "Your request is handled confidentially. You'll receive a confirmation by email.")}</p>
            </form>
          </div>
          <aside className="lg:col-span-5"><div className="bg-card border border-champagne/15 p-8"><div className="eyebrow mb-4">{tr("Direkter Kontakt", "Direct contact")}</div><h3 className="font-display text-2xl text-vanilla mb-6">{tr("Lieber persönlich?", "Prefer to reach me directly?")}</h3><ul className="space-y-5"><li className="flex items-start gap-4"><Mail className="text-champagne mt-1" size={18} /><div><div className="eyebrow mb-1">{tr("E-Mail", "Email")}</div><a href="mailto:Lady-vanillaice@gmx.net" className="text-vanilla hover:text-champagne transition">Lady-vanillaice@gmx.net</a></div></li><li className="flex items-start gap-4"><MessageCircle className="text-champagne mt-1" size={18} /><div><div className="eyebrow mb-1">WhatsApp</div><a href="https://wa.me/4915170568230" target="_blank" rel="noopener noreferrer" className="text-vanilla hover:text-champagne transition">+49 151 70568230</a></div></li><li className="flex items-start gap-4"><MessageCircle className="text-champagne mt-1" size={18} /><div><div className="eyebrow mb-1">Telegram</div><a href="https://t.me/ladyvanillaice" target="_blank" rel="noopener noreferrer" className="text-vanilla hover:text-champagne transition">@ladyvanillaice</a></div></li><li className="flex items-start gap-4"><MapPin className="text-champagne mt-1" size={18} /><div><div className="eyebrow mb-1">{tr("Standort", "Location")}</div><div className="text-vanilla">{tr("München und Umgebung", "Munich and surroundings")}</div></div></li></ul><div className="my-8"><span className="hairline" /></div><p className="text-xs text-vanilla/55 leading-relaxed">{tr("Termine ausschließlich nach vorheriger Vereinbarung. Spontane Anfragen ohne Vorgespräch werden nicht beantwortet.", "Appointments exclusively by prior arrangement. Spontaneous inquiries without a preliminary talk will not be answered.")}</p></div></aside>
        </div>
      </section>
    </>
  );
