...
function atNoon(date: string | null | undefined) {
...
export const updateBookingAccounting = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const cashDate = data.fully_paid && data.bar > 0
      ? (data.cash_received_at || completedDate || today)
      : data.cash_received_at;
    const depositDate = data.anzahlung > 0 ? data.anzahlung_paid_at : null;

    const bookingStatus = data.status === "cancelled"
      ? "cancelled"
      : data.status === "completed" || data.fully_paid
        ? "confirmed"
        : "confirmed";
