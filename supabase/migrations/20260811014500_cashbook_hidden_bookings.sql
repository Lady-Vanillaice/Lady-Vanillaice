...
create policy "Admins can view hidden cashbook bookings"
  on public.cashbook_hidden_bookings for select
  to authenticated
  using (private.has_role(auth.uid(), 'admin'::app_role));

drop policy if exists "Admins can hide cashbook bookings" on public.cashbook_hidden_bookings;
create policy "Admins can hide cashbook bookings"
  on public.cashbook_hidden_bookings for insert
  to authenticated
  with check (private.has_role(auth.uid(), 'admin'::app_role));

drop policy if exists "Admins can restore cashbook bookings" on public.cashbook_hidden_bookings;
create policy "Admins can restore cashbook bookings"
  on public.cashbook_hidden_bookings for delete
  to authenticated
  using (private.has_role(auth.uid(), 'admin'::app_role));
