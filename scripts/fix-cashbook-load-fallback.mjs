...
if (!backend.includes("cashbookBookingFallback")) {
  ...
  if (!before) throw new Error("Cashbook load fallback could not find the booking query block.");
  backend = backend.replace(before, after);
  writeFileSync(backendPath, backend);
  console.log("Cashbook retries with legacy-safe all-status booking queries instead of failing completely.");
} else {
  ...
  writeFileSync(backendPath, backend);
  console.log("Cashbook booking fallback already active and preserves all statuses.");
}
