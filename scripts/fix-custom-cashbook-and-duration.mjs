...
function patch(path, before, after, label) {
  ...
  if (!text.includes(before)) {
    console.warn(`[custom-cashbook] skipped missing target: ${label}`);
    return;
  }
...
patch(
  ...
  `  const [error, setError] = useState<string | null>(null);\n  const [success, setSuccess] = useState(false);\n\n  const productionMinutes = useMemo(() => {\n    if (!start || !end) return 0;\n    const [sh, sm] = start.split(":").map(Number);\n    const [eh, em] = end.split(":").map(Number);\n    let minutes = eh * 60 + em - (sh * 60 + sm);\n    if (minutes <= 0) minutes += 24 * 60;\n    return minutes;\n  }, [start, end]);\n  const productionDurationLabel = productionMinutes >= 60\n    ? String(Math.floor(productionMinutes / 60)) + " Std." + (productionMinutes % 60 ? " " + String(productionMinutes % 60) + " Min." : "")\n    : String(productionMinutes) + " Min.";`,
  "production duration calculation",
);
optionalPatch(customForm, `<label className="eyebrow block mb-1">Anzahl Bilder (optional)</label>`, `<label className="eyebrow block mb-1">Anzahl fertige Bilder (optional)</label>`);
optionalPatch(customForm, `<label className="eyebrow block mb-1">Video in Minuten (optional)</label>`, `<label className="eyebrow block mb-1">Dauer des fertigen Videos (Minuten, optional)</label>`);
optionalPatch(
  customForm,
...
patch(cashbookLib, `      const receivedCash = cashDate ? plannedCash : 0;`, `      const receivedCash = isCustomContent ? 0 : cashDate ? plannedCash : 0;`, "no custom cash/rest payment");
