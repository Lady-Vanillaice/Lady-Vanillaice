...
const importAnchor = 'import { listBookingRestPaymentMethods, updateBookingRestPaymentMethod } from "@/lib/rest-payment.functions";';
...
text = text.replace(
  ...
  '  const incomeActions = (e: CashBookEntry) => e.source === "booking" ? (\n    <div className="flex items-center gap-3">\n      <button onClick={() => setEditing(e)} className="text-champagne text-xs uppercase">Bearbeiten</button>\n      <button onClick={() => e.booking_id && confirm("Diesen Eintrag nur aus dem Kassenbuch entfernen? Der Termin und die Kundendaten bleiben erhalten.") && hideBookingMut.mutate(e.booking_id)} aria-label="Aus Kassenbuch entfernen" title="Aus Kassenbuch entfernen"><Trash2 size={15} /></button>\n    </div>\n  ) : (',
);

const markers = [
  'hideBookingFromCashbook',
  'cashbook-hidden-bookings',
  'hiddenBookingSet.has(e.booking_id)',
  'Aus Kassenbuch entfernen',
];
for (const marker of markers) {
  if (!text.includes(marker)) throw new Error(`Kassenbuch-Löschfunktion konnte nicht eingebaut werden: ${marker}`);
}

if (text !== before) {
  writeFileSync(path, text);
  console.log("Cashbook booking delete option added.");
} else {
  console.log("Cashbook booking delete option already present.");
}
